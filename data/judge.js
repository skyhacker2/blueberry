...
exports.getByRandom = function(chapter_id, num, callback) {
	connection.query("SELECT * FROM question.judge WHERE chapter_id=? ORDER BY rand() LIMIT ?", 
...
exports.getAllExcept = function(chapter_id, excepts, callback) {
	connection.query("SELECT * FROM question.judge WHERE chapter_id = ?", [chapter_id], function(err, rows) {
		if(err) {
			consolg.log(err);
		} else {
...
exports.getByIds = function(chapter_id, ids, callback) {
	var queryStr = "SELECT * FROM question.judge WHERE chapter_id = " + chapter_id + " AND ";
